...
- **User's proposed direction**: rethink the feature from scratch (chose /10x-frame
  over a prompt tweak).
- **Pre-dispatch narrowing**: redundancy is in **both** the items list *and* the
  summary sentence (whole panel restates the dashboard); the missing decision
  support is **all three** of — what to do first (priority/urgency), *why* (the
...
1. **Payload — what the model receives** — if it lacked the facts to add value,
   it could only restate.  ← *not* the user's framing, worth ruling out
...
3. **Output schema — what the model is allowed to return** — caps the *ceiling*
...
| 2. Prompt forbids synthesis | `SYSTEM_PROMPT`: *"Restate the engine's plan… Do NOT invent products, change quantities, or recompute anything… Return only the weekly_summary field."* (`restocking-summary.ts:25-30`). Explicitly instructs restatement, not decision-support. | STRONG |
...
- "All three decision gaps matter (priority, why, artifact)" → the wanted value
  is *synthesis* (ordering + reasoning + a consolidated artifact), none of which
...
Prioritizing, explaining, and consolidating are *synthesis over engine-computed
values* — they do not invent or alter a quantity, state, or selection — so the
...
> *restate* the engine's output (restate-only prompt + single-prose-string schema
> + a render that re-lists the tiles), so it can never do the one job the
> unordered tiles can't — turn the engine's facts into a *prioritized, explained,
> act-on-it weekly decision*. The data to do this is already in the payload; the
> three coupled constraints (prompt, schema, render) are what cap it at
> "rewording the cards."

Addressing this means letting the AI synthesize a *decision* — order the items by
...
AI contract from *restate* to *prioritize + explain + consolidate* — a richer
output schema (ordered items with per-item rationale + a headline/action view), a
prompt that asks for a weekly *decision* grounded in the supplied engine facts,
