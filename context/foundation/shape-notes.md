...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "data trapped in platform (sales history without interpretation) + decision paralysis (can't confidently act on raw numbers)"
    - topic: "core insight"
      decision: "velocity classification, not more charts — the app applies a rule the platform omits: is this item fast or slow relative to its own history and current stock level?"
    - topic: "primary persona scope"
      decision: "solo operator running a single store — one person, one shop, manages everything"
    - topic: "auth model"
      decision: "email + password or OAuth login; server-side accounts; multi-device"
    - topic: "user roles"
      decision: "flat model — one account per store owner, full access, no role separation at MVP"
    - topic: "MVP timeline"
      decision: "3 weeks after-hours; full flow: auth + product entry + sales entry + classification + recommendation + dashboard"
    - topic: "secondary goal"
      decision: "multi-product dashboard grouped by classification state"
    - topic: "guardrails"
      decision: "1) strict data isolation per account; 2) explicit 'insufficient data' state instead of misleading classification"
    - topic: "insufficient data threshold"
      decision: "7 days of non-overlapping sales history"
    - topic: "slow-mover threshold"
      decision: "days_of_stock > 90 days or velocity < 0.1 units/day"
    - topic: "buffer days"
      decision: "default 7 days, owner-overridable per product"
  frs_drafted: 11
  quality_check_status: accepted
---

# StockHelper — Product Requirements Document

**Seed idea:** A web application that helps small e-commerce store owners track product sales velocity, automatically classifies items as understocked or slow-moving, and recommends restocking or promotional actions.
...
Store owners authenticate via email + password or OAuth (e.g., Google). Accounts are server-side; data is accessible from any device.
...
- **No shop platform integrations** — sales data is entered manually. No Shopify/WooCommerce/Etsy API sync, OAuth connections to shop platforms, or webhook listeners.
...
| Business Logic | present — formulas and thresholds fully specified |
